...
public class MainPage {
    ...
    public void selectAndCheckMenuItem() {
        /*
        Локатор для выбора элементов меню (поочередно) оставил прежним. Согласен, что он
        достаточно сложен и при изменении верстки может придется его корректировать.
        При переборе элементов меню в цикле обнаружил, что при переходе в пункт меню "Каталог"
        аттрибуты элементов меню (li) полностью меняются. В частности, на пункте "Заказы" id у элемента
        есть, а после перехода в "Клиенты" у всех элементов меню id уже нет. На класс тоже
        "заточиться" нельзя, так как он тоже полностью меняется на том же пункте меню.
        Смог подобрать только такой локатор, работающий стабильно для поочередного выбора пунктов.
         */
